-- Supabase enables pg-safeupdate for API queries, so UPDATE/DELETE statements
-- must carry a WHERE clause even inside SECURITY DEFINER functions. Migrations
-- 006 and 007 shipped WHERE-less runtime/reset updates; this forward migration
-- replaces those functions on already-migrated projects. Fresh projects get the
-- corrected definitions directly from 006/007.
