...
`/var/lib/model-server-hourly-probe/latest.json`。记录包含本次检测运行 ID、最终状态、时间、
总耗时，以及健康检查、鉴权、额度恢复和排程请求各阶段的路由、HTTP 状态、耗时、
安全业务错误码、请求 ID 与响应请求 ID。失败邮件也包含检测运行 ID 和可用的请求 ID，
便于关联公网账号 API、内部规划服务及模型代理日志。记录不包含令牌、供应商原始错误、
异常原文、请求正文或模型输出。

网络错误会区分 DNS、TLS、连接失败、超时和其他网络错误；账号 API 对失败请求记录路由模板、
HTTP 状态、安全业务错误码与请求 ID，账号后端另行记录 Supabase 或内部规划服务的操作名、
安全异常类别、尝试次数、耗时、上游 HTTP 状态和安全业务错误码。两侧日志都不记录上游响应正文、
账号标识、会话、支持码或凭据。
...
恢复邮件主题为 `[DayMosaic] AI 规划服务已恢复`，正文包含服务名称、检测时间、首次异常时间、
`真实排程已通过：08:00–08:30。` 和本次检测耗时。测试邮件主题为
`[DayMosaic] AI 规划告警邮箱验证`，正文说明它是配置验证邮件且没有触发模型调用。
邮件正文不包含 URL，并且均不包含用户日程原文、模型原始输出或凭据。
