...
The repair changes only the model-facing add schema and its compilation to
existing solver operations. The public API, scheduler algorithm, shared golden
responses, iOS code, production model selection, and deployment are unchanged.
...
| Shared client/server goldens | All 24 fixture byte hashes and public candidate outputs unchanged | `test_time_fragment_planner.py::test_time_fragment_planner_golden_fixture`, `test_golden_fixture_manifest_matches_exact_bytes_and_case_set` |
| Current full-day report | All 12 stated anchors preserved; combined commute 19:30–20:00; 24:00 sleep remains a single-day boundary error, not noon | `test_time_fragment_clock_extraction.py::test_entire_reported_day_preserves_clock_anchors_and_flags_only_midnight_boundary` |
...
The focused campaign selects these four affected test modules and one exact
parameter-forwarding selector, not the full repository test suite. The few
auth/envelope-only API tests in the selected module run incidentally. No iOS
source, resources, project inputs, or public golden bytes change; therefore this
backend-only campaign selects zero XCUITests. Compilation is checked separately.
...
duration starting at 24:00 cannot be scheduled on the selected day.
