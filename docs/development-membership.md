...
Members receive 50 successful AI planning requests per calendar day, resetting at 00:00 Asia/Shanghai using server time. Free installations retain their existing cumulative quota (default 30). Member use does not consume the free balance. Turning membership off and on preserves that day's usage. Failed requests are refunded, and previously consumed request IDs remain idempotent across modes and days.
