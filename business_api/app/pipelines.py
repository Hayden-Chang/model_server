...
PIPELINES: dict[str, Pipeline] = {
    ...
    "time-fragment-plan-v2": Pipeline(
        ...
        system_prompt=(
            ...
            "Interpret informal English directly, including fit in, squeeze in, contractions, "
            "and lower-case references to current titles. Never rewrite the user's text. "
            "For example, 'fit in 45 mins for task B' requests an add of task B with durationSlots=3; "
            "do not place or split it yourself, and do not modify an existing task A. "
            ...
            "negative or keep-unchanged phrase as authorization. For a named protected target, also "
            "include authorization={action, targetItemId, targetText, sourceText, affirmative}: "
            "action must match the requested operation; targetItemId must match the existing target; "
            "targetText is its title or ID as named in the source (case differences are OK); "
            "sourceText must equal authorizationText; affirmative is your semantic judgment of the "
            "complete original request, including negation, cancellation, and keep-unchanged clauses. "
            "Do not emit an operation for a negated or ambiguous change. "
            "Existing move placement slots are "
            ...
            "or combine source actions and need not occur verbatim in sourceText. Preserve an explicitly "
            "named task, including labels such as 'task C'; do not abbreviate that name to 'C'. "
            "A timed object requires "
            ...
            "must include English clock context, e.g. 'at 3' or 'to 4', not a bare digit; "
            "AM/PM, when explicit, must be preserved. startEvidence "
